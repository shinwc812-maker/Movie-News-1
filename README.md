...
커뮤니티 반응은 공식 기사와 분리해 `data/community.json`에 저장하며, 익스트림무비, 더쿠/디시인사이드 직접 검색, 네이버카페 공개 검색, YouTube 검색 API를 사용합니다. 인스타그램·X(트위터)·페이스북 등 SNS는 약관·법적 리스크로 수집하지 않습니다. 영화명은 `와일드 씽`/`와일드씽`처럼 띄어쓰기 변형까지 함께 검색합니다.
...
익스트림무비, 더쿠, 디시인사이드, 네이버카페 공개 검색은 별도 키 없이 동작합니다. Naver Search Open API와
...
공개 검색 결과를 직접 수집합니다. (인스타그램·X·페이스북 등 SNS는 수집하지 않습니다.)
...
우상단 검색창에 키워드를 입력하면 해당 키워드를 포함한 핵심 큐레이션·시장동향·커뮤니티 반응만
필터링되고 나머지 섹션은 숨겨집니다(`전체 보기`로 해제). `⚙️ 설정` 버튼으로 섹션별 표시/숨김을
켜고 끌 수 있으며, 선택은 브라우저에 저장됩니다.
