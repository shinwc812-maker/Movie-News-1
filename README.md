...
KOBIS 박스오피스, 실시간 예매율 TOP 5, 영화/문화 정책 지원 공지를 자동 수집해
정적 HTML 대시보드를 생성합니다.

수집 매체: Variety, The Hollywood Reporter, Deadline, IndieWire, Rolling Stone (US, RSS) · 씨네21, 맥스무비 (KR, 스크래핑).
커뮤니티 반응은 공식 기사와 분리해 `data/community.json`에 저장하며, 익스트림무비, 네이버카페 공개 검색, X/Twitter 공개 웹 검색, YouTube 검색 API를 사용합니다.
...
Naver API 인증이 실패해도 공개 검색 fallback은 계속 실행됩니다. X/Twitter는 로그인 없이 노출되는
공개 검색 결과만 수집하므로, 검색 결과가 막히는 날에는 0건일 수 있습니다.
